...
class ColorNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
        ...
        if deep_stem:
            ...
            self.conv1 = nn.Sequential(*[
                nn.Conv2d(in_chans * 2, stem_chs[0], 3, stride=2, padding=1, bias=False),
            ...
            self.conv1 = nn.Conv2d(in_chans*2, inplanes, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def forward_features(self, x):
        x = torch.cat( 
                (x, 
                 torch.mean(x, dim=(2, 3), keepdim=True).expand(-1, -1, *x.shape[2:])),
                dim=1)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.act1(x)
...
def _create_colornet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ColorNet, variant, pretrained,
        default_cfg=default_cfgs[variant],
        **kwargs)


@register_model
def colornet50(pretrained=False, **kwargs):
    if pretrained:
        raise Exception('Not sure how to integrate with pretrained system.')
    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3],  **kwargs)
    return _create_colornet('resnet50', pretrained, **model_args)


@register_model
def colornet18(pretrained=False, **kwargs):
    if pretrained:
        raise Exception('Not sure how to integrate with pretrained system.')
    model_args = dict(block=BasicBlock, layers=[2, 2, 2, 2],  **kwargs)
    return _create_colornet('resnet18', pretrained, **model_args)
